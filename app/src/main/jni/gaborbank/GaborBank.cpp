...
namespace emotime {
  ...
  class FilterBankParallel : public cv::ParallelLoopBody
      {
      ...
      private:
          ...
          Mat& img;
      public:
          FilterBankParallel(std::vector<emotime::GaborKernel*> inputBank, Size inputSize,
          cv::Size& inputFeatSize, Mat& inputSrc, Mat& inputDst, Mat& inputImg)
                      : bank(inputBank), size(inputSize), featSize(inputFeatSize),
                        src(inputSrc), dest(inputDst), img(inputImg){}
          ...
          virtual void operator()(const cv::Range& range) const
          {
              for(int k = range.start; k < range.end; k++)
              {
                  ...
                  clock_t tmInitVars = clock();
                   __android_log_print(ANDROID_LOG_DEBUG,
                      LOG_TAG,
                      "Init vars in %f seconds.", double(tmInitVars - tmBegin) / CLOCKS_PER_SEC);
                  filter2D(src, freal, CV_32F, real);
                  filter2D(src, fimag, CV_32F, imag);
                  clock_t tmFilterImage = clock();
                  __android_log_print(ANDROID_LOG_DEBUG,
                      LOG_TAG,
                      "Filter 2D in %f seconds.", double(tmFilterImage - tmInitVars) / CLOCKS_PER_SEC);
                  ...
                  for (int i = 0; i<(unsigned int) featSize.height; i++) {
                    for (int j = 0; j<(unsigned int)featSize.width; j++) {
  ...
  void GaborBank::fillGaborBankEmpiric(double nwidths, double nlambdas, double nthetas){
    ...
    Mat GaborBank::filterImage(cv::Mat & src, cv::Size & featSize) {
      ...
      __android_log_print(ANDROID_LOG_DEBUG,
          LOG_TAG,
          "Bank size is %d", bank.size());


      cv::parallel_for_(cv::Range(0, bank.size()), FilterBankParallel(bank, size, featSize, src,
        dest, image));
